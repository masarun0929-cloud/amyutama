...
`js/config.js` はサイトの表示名、リンク、チャンネル、モード表示をまとめた設定ファイルです。
...
1. 使いたいファイルを選ぶ
2. その内容で `js/config.js` を置き換える
3. `replace_with_...` の値を自分用に変える
4. 必要なら `index.html` の説明文も1チャンネル向けに変える

PowerShell例:

```powershell
Copy-Item js\config.single-channel-listener-only.js js\config.js
```
